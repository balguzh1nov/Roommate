...
class MatchView: UIView {
    ...
    var cardUID: String! {
        didSet {
            ...
            query.document(cardUID).getDocument { (snapshot, error) in
                ...
                self.descriptionLabel.text = "Вы и \(user.name ?? "") лайкнули друг-друга"
    ...
    fileprivate let descriptionLabel: UILabel = {
        let label = UILabel()
        label.text = "Вы и X лайкнули друг-друга" //You and X have liked\neach other
    ...
    fileprivate let keepSwipingButton: KeepSwipingButton = {
        ...
        button.setTitle("Продолжить просмотр", for: .normal)
        button.setTitleColor(.white, for: .normal)
        return button
    }()
