...
class MessagesNavBar: UIView {
    ...
    let nameLabel = UILabel(text: "ИМЯ ПОЛЬЗОВАТЕЛЯ", font: .systemFont(ofSize: 16))
    
    let backButton = UIButton(image: #imageLiteral(resourceName: "back").withRenderingMode(.alwaysOriginal), tintColor: #colorLiteral(red: 0.03529411765, green: 0.5176470588, blue: 0.8901960784, alpha: 1))
//    let flagButton = UIButton(image: #imageLiteral(resourceName: "flag").withRenderingMode(.alwaysOriginal))
    ...
    init(match: Match) {
        ...
        hstack(backButton.withWidth(50),
               middleStack).withMargins(.init(top: 0, left: 4, bottom: 0, right: 16))
    }
